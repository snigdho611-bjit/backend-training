...
class Product {
    async getAll(req, res) {
        try {
            ...
            return res
                ...
                .send(success("Successfully got all products", { products: allProducts, total: allProducts.length }));
        } catch (error) {
            console.log(error);
    ...
    async create(req, res) {
        try {
            ...
            if (newProduct) {
                return res.status(HTTP_STATUS.OK).send(success("Successfully added product", newProduct));
            }
        } catch (error) {
            console.log(error);
            return res.status(HTTP_STATUS.INTERNAL_SERVER_ERROR).send(failure("Internal server error"));
